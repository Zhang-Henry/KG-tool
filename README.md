...
1. 首先创建图谱名。如果图谱名已经存在于数据库，则会提示名字重复。否则将图谱名称存入缓存。图谱名会作为当前图谱所有实体和关系的一个属性存进Neo4j，以区分不同图谱的数据。
![创建图谱](./image/创建图谱.png)
...
## 4. 基于聊天(对话)语音构建知识图谱
- 基于对话提取关系——DialogueRE
- 针对医患对话提取关键信息，e.g. 病历, 保险信息, 用药信息: [MIE](https://aclanthology.org/2020.acl-main.576/)
