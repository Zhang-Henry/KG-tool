...
from kgproject.ner.model_utils import tensorized, sort_by_lengths, cal_lstm_crf_loss, save_model
from kgproject.ner.model.bilstm import BiLSTM
...
class BILSTM_Model(object):
    def __init__(self, vocab_size, out_size):
        ...
        if os.path.isfile('./kgproject/ner/save_model' + '/bilstm_crf.ckpt'):
            self.model.load_state_dict(torch.load('./kgproject/ner/save_model'+ '/bilstm_crf.ckpt', map_location='cpu'))  # 修改为cpu
    ...
    def validate(self, dev_word_lists, dev_tag_lists, word2id, tag2id):
        ...
        with torch.no_grad():
            ...
            if val_loss < self._best_val_loss:
                print("保存模型...")
                save_model(self.model, './save_model' + '/bilstm_crf.ckpt')
